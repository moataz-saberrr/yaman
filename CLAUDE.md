...
- **Custom CSS:** YAMAN homepage/section CSS goes in `assets/custom_yaman.css` (loaded globally from `layout/theme.liquid`, after the `css-variables` snippet), scoped per section. A merge from `main` (dev2) also added `assets/custom-general.css` (global), `assets/custom-product.css` (product templates with a suffix) and `assets/custom-collection.css` (collection templates, no suffix) — keep YAMAN homepage styles in `custom_yaman.css` and coordinate with dev2 before using theirs.
- **Brand fonts** are now bundled (DIN Next, in `assets/`): the `@font-face` rules + `--font-body-family` (`'DIN Next LT'`) / `--font-heading-family` (`'DIN Next Arabic'`) variables live in `assets/base.css` and apply to `body` / `h1–h3`, so new sections inherit them — do **not** hardcode font-family. (Caveat: that `@font-face` uses Liquid inside static CSS — see `YAMAN_PROJECT_PLAN.md` §11.)
