...
- **Use logical CSS properties for horizontal spacing/positioning** (RTL/translation readiness). Prefer `padding-inline` / `padding-inline-start` / `padding-inline-end`, `margin-inline*`, `inset-inline-start` / `inset-inline-end`, `border-inline-start` / `border-inline-end`, and `gap`. Avoid physical `left` / `right` / `padding-left` / `padding-right` / `margin-left` / `margin-right` unless there's a strong reason. The site may be translated (Arabic/RTL) later, so all future custom YAMAN sections should default to logical properties.
